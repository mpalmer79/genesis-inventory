...
function inventorySpecificityScore(query, vehicles) {
  const normalized = query.toLowerCase();
  let score = 0;

  for (const model of catalogValues(vehicles, 'model')) {
    if (new RegExp(`\\b${escapeRegExp(String(model).toLowerCase()).replace(/\\ /g, '\\s+')}\\b`).test(normalized)) {
      score += 12;
      break;
    }
  }
  ...
  for (const make of catalogValues(vehicles, 'make')) {
    if (new RegExp(`\\b${escapeRegExp(String(make).toLowerCase())}\\b`).test(normalized)) {
      score += 6;
      break;
    }
  }

  if (/\b(?:awd|fwd|rwd|4wd|4x4)\b/i.test(query)) score += 3;
  if (/\b(?:new|used|certified|in-stock|in-transit)\b/i.test(query)) score += 2;
  if (/\b\d+(?:\.\d+)?k miles\b/i.test(query)) score += 2;
  return score;
}
...
export function normalizeVoiceInventoryQuery(transcript, vehicles = []) {
  ...
  query = query
    // Speech recognition frequently hears the inventory term "used" as "use".
    // In voice inventory queries, treat the standalone word "use" as USED stock type.
...
export function chooseVoiceInventoryQuery(transcripts, vehicles = []) {
  const candidates = [...new Set((transcripts || []).map(cleanTranscript).filter(Boolean))]
    .map((transcript, index) => {
      const normalized = normalizeVoiceInventoryQuery(transcript, vehicles);
      return {
        ...normalized,
        score: inventorySpecificityScore(normalized.query, vehicles) - index * 0.05
      };
    });

  candidates.sort((a, b) => b.score - a.score);
  return candidates[0] || { heard: '', query: '', changed: false, score: 0 };
}

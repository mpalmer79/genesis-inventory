...
const TRIM_RESERVED_WORDS = new Set([
  'awd', 'fwd', 'rwd', '4wd', '4x4', 'all', 'wheel', 'drive', 'front', 'rear', 'four',
  'black', 'white', 'gray', 'grey', 'silver', 'blue', 'red', 'green', 'brown', 'beige', 'gold', 'orange', 'purple',
  'suv', 'sedan', 'crossover', 'electric', 'electrified', 'new', 'used', 'certified', 'cpo',
  'automatic', 'manual', 'auto', 'transmission', 'engine', 'turbo', 'liter', 'litre'
]);
...
function matchInventoryIdentifier(normalizedQuery, vehicles) {
  ...
  for (const vehicle of vehicles || []) {
    const stockNumber = normalizeIdentifier(vehicle?.stockNumber);
    if (stockNumber && tokens.includes(stockNumber)) {
      return { field: 'stockNumber', value: vehicle.stockNumber, vehicle };
    }
  }

  for (const vehicle of vehicles || []) {
    const vin = normalizeIdentifier(vehicle?.vin);
    if (vin && tokens.includes(vin)) {
      return { field: 'vin', value: vehicle.vin, vehicle };
    }
  }
...
function canonicalDrivetrain(value) {
  ...
  if (/\bawd\b/.test(normalized) || /\ball wheel drive\b/.test(normalized)) return 'AWD';
  if (/\bfwd\b/.test(normalized) || /\bfront wheel drive\b/.test(normalized)) return 'FWD';
  if (/\brwd\b/.test(normalized) || /\brear wheel drive\b/.test(normalized)) return 'RWD';
  if (/\b4wd\b/.test(normalized) || /\b4x4\b/.test(normalized) || /\bfour wheel drive\b/.test(normalized)) return '4WD';
...
function matchMake(normalizedQuery, values) {
  const direct = matchCatalogValue(normalizedQuery, values, true);
  if (direct) return direct;

  for (const [alias, canonicalMake] of MAKE_ALIASES) {
    const availableMake = values.find((value) => normalizeText(value) === normalizeText(canonicalMake));
    if (!availableMake) continue;
    const escapedAlias = escapeRegExp(alias).replace(/\\ /g, '\\s+');
    if (new RegExp(`\\b${escapedAlias}\\b`, 'i').test(normalizedQuery)) return availableMake;
  }
...
function trimDescriptorTokens(trim) {
  return normalizeText(trim)
    .split(' ')
    .map((token) => token.trim())
    .filter((token) => token
      && !TRIM_RESERVED_WORDS.has(token)
      && !/^[1-9](?:\.\d)?(?:l|t)?$/.test(token)
      && !/^\d+$/.test(token));
}

function catalogTrimPhrases(vehicles) {
  const phrases = new Set();

  for (const trim of catalogValues(vehicles, 'trim')) {
    const tokens = trimDescriptorTokens(trim);
    for (let start = 0; start < tokens.length; start += 1) {
      for (let length = 1; length <= Math.min(3, tokens.length - start); length += 1) {
...
function matchTrimPhrase(normalizedQuery, vehicles) {
  for (const phrase of catalogTrimPhrases(vehicles)) {
    const escaped = escapeRegExp(phrase).replace(/\\ /g, '\\s+');
    if (new RegExp(`\\b${escaped}\\b`, 'i').test(normalizedQuery)) return phrase;
  }
  return null;
}
...
export function parseInventoryQuery(query, vehicles = []) {
  ...
  const identifier = matchInventoryIdentifier(normalized, vehicles);
  if (identifier) {
    const vehicle = identifier.vehicle;
    filters[identifier.field] = identifier.value;
    if (vehicle.condition) filters.condition = vehicle.condition;
    if (vehicle.availability) filters.availability = vehicle.availability;
    if (vehicle.make) filters.make = vehicle.make;
    if (vehicle.model) filters.model = vehicle.model;
    labels.push(identifier.field === 'stockNumber' ? `Stock ${identifier.value}` : `VIN ${identifier.value}`);
  ...
  const drivetrain = canonicalDrivetrain(normalized);
  if (drivetrain) {
    filters.drivetrain = drivetrain;
    labels.push(drivetrain);
  }

  const make = matchMake(normalized, catalogValues(scopedVehicles, 'make'));
  if (make) {
    filters.make = make;
    labels.push(make);
  }
  ...
  const trimPool = scopedVehicles.filter((vehicle) => {
    if (filters.make && normalizeText(vehicle.make) !== normalizeText(filters.make)) return false;
    if (filters.model && normalizeText(vehicle.model) !== normalizeText(filters.model)) return false;
    return true;
  });
  const trimPhrase = matchTrimPhrase(normalized, trimPool);
  if (trimPhrase) {
    filters.trimPhrase = trimPhrase;
    labels.push(`Trim: ${trimPhrase.replace(/\b\w/g, (letter) => letter.toUpperCase())}`);
  }
  ...
  const mileageMatch = normalized.match(/(?:under|below|less than|up to|max(?:imum)?(?: of)?)\s*([\d,.]+k?)\s*(?:miles?|mi|m)\b/i);
  if (mileageMatch) {
  ...
  const stopWords = new Set([
    ...
    'do', 'does', 'we', 'have', 'got', 'looking', 'look',
    'under', 'below', 'less', 'than', 'up', 'to', 'over', 'above', 'more', 'at', 'least', 'new', 'used', 'pre', 'owned',
    'certified', 'cpo', 'in', 'stock', 'transit', 'available', 'now', 'price', 'dollar', 'dollars', 'miles', 'mile', 'mi', 'm',
...
function matchesFilters(vehicle, filters) {
  if (filters.stockNumber && normalizeIdentifier(vehicle.stockNumber) !== normalizeIdentifier(filters.stockNumber)) return false;
  if (filters.vin && normalizeIdentifier(vehicle.vin) !== normalizeIdentifier(filters.vin)) return false;
  ...
  if (filters.drivetrain && canonicalDrivetrain(vehicle.drivetrain) !== filters.drivetrain) return false;
  ...
  if (filters.trimPhrase) {
    const trim = normalizeText(vehicle.trim);
    if (!trim.includes(filters.trimPhrase)) return false;
  }

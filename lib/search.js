const MODEL_PATTERN = /\b(electrified\s+gv70|gv80\s+coupe|gv80|gv70|gv60|g90|g80|g70)\b/i;
...
function extractRange(query, nouns) {
  ...
  const under = query.match(new RegExp(`(?:under|below|less than|up to|max(?:imum)?(?: of)?)\\s*\\$?([\\d,.]+k?)\\s*(?:${nounPattern})?`, 'i'));
  const over = query.match(new RegExp(`(?:over|above|more than|at least|min(?:imum)?(?: of)?)\\s*\\$?([\\d,.]+k?)\\s*(?:${nounPattern})?`, 'i'));
  return {
    max: parseCompactNumber(under?.[1]),
    min: parseCompactNumber(over?.[1])
  };
}
...
export function parseInventoryQuery(query) {
  ...
  if (/\bsuv\b|\bcrossover\b/.test(normalized)) {
    filters.bodyStyle = 'suv';
    labels.push('SUV');
  } else if (/\bsedan\b/.test(normalized)) {
...
function matchesFilters(vehicle, filters) {
  ...
  const effectivePrice = Number(vehicle.price ?? vehicle.msrp);
  if (filters.minPrice != null && (!Number.isFinite(effectivePrice) || effectivePrice < filters.minPrice)) return false;
  if (filters.maxPrice != null && (!Number.isFinite(effectivePrice) || effectivePrice > filters.maxPrice)) return false;
...
export function searchInventory(vehicles, query) {
  ...
  scored.sort((a, b) => {
    ...
    const aPrice = Number(a.vehicle.price ?? a.vehicle.msrp) || Number.MAX_SAFE_INTEGER;
    const bPrice = Number(b.vehicle.price ?? b.vehicle.msrp) || Number.MAX_SAFE_INTEGER;

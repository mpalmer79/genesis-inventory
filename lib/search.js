...
export function parseInventoryQuery(query, vehicles = []) {
  ...
  const make = matchCatalogValue(normalized, catalogValues(vehicles, 'make'));
  if (make) {
    filters.make = make;
    labels.push(make);
  }
  ...
  const modelPool = make
    ? (vehicles || []).filter((vehicle) => normalizeText(vehicle.make) === normalizeText(make))
    : vehicles;
  ...
  if (model) {
    ...
    if (!filters.make && vehicles.length) {
      const matchingMakes = catalogValues(
        vehicles.filter((vehicle) => normalizeText(vehicle.model) === normalizeText(model)),
        'make'
      );

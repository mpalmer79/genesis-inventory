...
test('recognizes plural vehicle makes in conversational queries', () => {
  const parsed = parseInventoryQuery('do we have any used Hondas', inventory.vehicles);
  assert.equal(parsed.filters.condition, 'used');
  assert.equal(parsed.filters.make, 'Honda');

  const result = searchInventory(inventory.vehicles, 'do we have any used Hondas');
  assert.ok(result.results.length > 0, 'Used Hondas should return production matches');
  assert.ok(result.results.every((vehicle) => vehicle.make === 'Honda'));
  assert.ok(result.results.every((vehicle) => ['used', 'certified'].includes(vehicle.condition)));
});

test('recognizes common spoken make aliases when the make exists in inventory', () => {
  const aliasVehicles = [...vehicles, {
    vin: 'MERCEDES666666666',
    stockNumber: 'M6001',
    condition: 'used',
    availability: 'in-stock',
    year: 2022,
  ...
  assert.equal(parseInventoryQuery('used Chevys', aliasVehicles).filters.make, 'Chevrolet');
  assert.equal(parseInventoryQuery('used Benzes', aliasVehicles).filters.make, 'Mercedes-Benz');
});
...
test('treats a recognized trim term as a hard filter', () => {
  const parsed = parseInventoryQuery('Prestige', vehicles);
  assert.equal(parsed.filters.trimPhrase, 'prestige');

  const result = searchInventory(vehicles, 'Prestige');
  assert.equal(result.results.length, 1);
  assert.equal(result.results[0].stockNumber, 'GM1002');
  assert.ok(result.results.every((vehicle) => /prestige/i.test(vehicle.trim || '')));
});

test('production Prestige search never returns non-Prestige inventory', () => {
  const result = searchInventory(inventory.vehicles, 'Prestige');
  assert.ok(result.results.length > 0, 'Prestige should return production matches');
  assert.ok(result.results.length < inventory.vehicles.length, 'Prestige must constrain the result set');
  assert.ok(result.results.every((vehicle) => /prestige/i.test(vehicle.trim || '')));
});

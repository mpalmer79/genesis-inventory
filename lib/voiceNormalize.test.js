...
const lexusMake = inventory.vehicles.find((vehicle) => String(vehicle.make).toLowerCase() === 'lexus')?.make || 'Lexus';
...
test('normalizes certified and pre-owned phrasing', () => {
  ...
  assert.equal(normalizeVoiceInventoryQuery('pre owned Lexus', inventory.vehicles).query, `used ${lexusMake}`);
});
